            ...
            {property?.units?.length > 0
              ? property?.units?.map((unit: any, idx: number) => (
                  <AccordionItem
                    value={`unit-${idx}`}
                    key={unit.id}
                    className="rounded-2xl bg-white overflow-hidden mb-4 border-2 border-secondary-bg !text-primary-bg"
                  >
                    ...
                    <AccordionContent className="mt-2 rounded-b-2xl px-6 py-5 border-secondary-bg shadow-sm">
                      <div className="grid grid-cols-2 md:grid-cols-2 gap-4 text-sm text-primary-bg capitalize">
                        <div>
                          <strong className="font-semibold">
                            Assigned Tenant
                          </strong>
                          <p className="pt-1">{unit.name ?? 'Not Assigned'}</p>
                        </div>
                        <div>
                          <strong className="font-semibold">Type</strong>{' '}
