            ...
            {property?.units?.length > 0
              ? property?.units?.map((unit: any, idx: number) => (
                  <AccordionItem
                    value={`unit-${idx}`}
                    key={unit.id}
                    className="rounded-2xl bg-white overflow-hidden mb-4 border-2 border-secondary-bg"
                  >
                    <AccordionTrigger className="px-6 py-6 text-[15px] font-medium text-gray-800 bg-white border-secondary-bg border-b-2 outline-none hover:outline-none hover:bg-secondary-bg transition rounded-t-2xl focus:outline-none focus:ring-0 focus-visible:ring-0">
