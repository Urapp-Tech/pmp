...
//   // const SCROLL_COOLDOWN = 1000; // ms (slower slide response)
//   // const WHEEL_THRESHOLD = 40; // minimum delta to trigger a slide
//   // ---- replace these values ----
// const SCROLL_COOLDOWN = 900;    // ms (slide step thoda slow)
// const WHEEL_THRESHOLD = 110;    // kitna wheel chahiye step ke liye (zyada = slow)
// const STEP_SCROLL_SCALE = 0.9;  // deltaY ko soften (smooth feel)
...
//     const nextY = currentY + (targetY - currentY) * 0.18; // tweak: smaller = slower
...
//   return () => {
//     window.removeEventListener('rento:enterManaged', onEnterManaged as EventListener);
//     window.removeEventListener('rento:leaveManaged', onLeaveManaged as EventListener);
//   };
...
//     // slow factor (0.3 = ~70% slower). Change 0.25–0.5 to taste
//     const scale = 0.3;

//     // clamp very large trackpad deltas
//     const dy = e.deltaY;
//     const moderated = Math.sign(dy) * Math.min(Math.abs(dy), 180);
...
//       0,
//       Math.min(docHeight - viewport, slowScrollState.current.targetY + moderated * scale)
//     );
...
//   const navBoundsRef = useRef<{ start: number; end: number }>({ start: 0, end: Number.POSITIVE_INFINITY });
...
//     const t1 = setTimeout(() => { calcBounds(); onScroll(); }, 300);
//     const t2 = setTimeout(() => { calcBounds(); onScroll(); }, 1000);
...
//     // lock after consuming to avoid rapid double-steps
...
//                     <motion.div
//                       layout
//                       className={`flex items-center gap-2 shrink-0 ${pinned
...
//           {/* Fixed Section Nav (right-bottom) */}
//           {/* <div className="fixed right-2 bottom-6 z-[1000] flex flex-col gap-2 items-end">
//             {[
...
//               const normalCls = ' bg-[#DFF4EC] text-[#242460] border-2 border-solid border-[#242460]/20  ';
